...
def _group_key(triplet_index: str, document_id: str) -> str:
    """Return a string key with triplet_index|document_id """
...
def fetch_original_vector(
    ...
    try:
        ...
        # if result["embeddings"][0] and len(result["embeddings"]) > 0:
...
def run_query_experiment(
    ...
    data            : merged_id_triplets_with_metadata.json data containing every information that gt_record and all_gt_records have
    ...
    for chunk_id in stable_chunks:
        registry.get_or_create(triplet_index, chunk_id)   # registers if new
        groups_in_query.add(_group_key(triplet_index, chunk_id))
        # all chunk targeted and the question to which they are associated are in the registry

    # ── Process each chunk ────────────────────────────────────────────────────
    chunk_sims: list[ChunkSimilarities] = []
    rotated_chunk_ids_written: set[str] = set()

    for chunk_id in stable_chunks: #chunk in stable_chunks: 
        tid       = chunk_id.split('|')[0] # triplet_index # chunk["triplet_index"] # chunk_triplet_index
        did       = chunk_id[-2] #chunk["document_id"]  # //
        pseq      = chunk_id[-1] #chunk["phrase_seq"]  # //
        ckey      = chunk_id #f"{tid}|{did}|{pseq}" 
        gkey      = _group_key(triplet_index, " ") #_group_key(tid, chunk_id) # _group_key(tid, did) #  --   key that will be hash to assign the rotation matrix
        R         = registry.get_or_create(triplet_index, " ") #registry.get_or_create(tid, did) #  -- all chunk in the group will have the same rotation matrix # need to be triplet_index so all document have a rotation determine by the document that is targeted by the question 
        seed      = registry._store[gkey]["seed"]

        # ── Get original embedding (from ChromaDB or re-embed) ────────────────
        
        # split chunk_id 
        orig_vec = fetch_original_vector(orig_collection, tid, chunk_id[-2], chunk_id[-1]) # ensure with debugger orig_vec = fetch_original_vector(orig_collection, tid, did, pseq)
        content = fetch_original_vector_content(orig_collection, tid, chunk_id[-2], chunk_id[-1])
        ...
        chroma_id = f"{triplet_index}_{chunk_id[-2]}_{chunk_id[-1]}" #  f"{tid}_{did}_{pseq}" #
        if chroma_id not in rotated_chunk_ids_written:
            rot_collection.upsert(
                ids        = [chroma_id],
                embeddings = [rot_vec.tolist()],
                documents  = [content], #[chunk["content"]],
                metadatas  = [{
                    "triplet_index": triplet_index, #tid,
                    "document_id":   chunk_id[-2], #did,
                    "phrase_seq":    chunk_id[-1], # pseq,
                    "group_key":     gkey,
                    "rotation_seed": seed,
                }],
            )
...
def run_experiment(
    ...
    with open(gt_path, encoding="utf-8") as fh:
        ...
        # Same with merged_id_triplets_with_metadata.json
    ...
    orig_coll     = _get_original_collection(ORIGINAL_CHROMA, ORIGINAL_COLLECTION) # collection with all access
    rot_coll      = _get_or_create_rotated_collection(ROTATED_CHROMA, ROTATED_COLLECTION) # collection with the rotation
